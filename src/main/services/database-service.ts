...
export class DatabaseService {
  ...
  public async clearAllMessages(): Promise<void> {
    try {
      this.runQuery("DELETE FROM messages");
  ...
  public async getAIResponses(messageId: string): Promise<AIResponse[]> {
    try {
      ...
      const rows = this.allQuery(sql, [messageId]);
      return rows.map((row) => ({
        id: row.response_id,
  ...
  public async getUser(slackId: string): Promise<any | null> {
    try {
      ...
      const row = this.getQuery(sql, [slackId]);
      return row || null;
    } catch (error) {
  ...
  public async getRipplingUser(email: string): Promise<RipplingUser | null> {
    try {
      ...
      const row = this.getQuery(sql, [email]);
      if (!row) return null;
  ...
  public async cleanup(retentionDays: number = 30): Promise<void> {
    try {
      ...
      const result = this.runQuery(sql, [cutoffDate.toISOString()]);
  ...
  public async getAnalytics(days: number = 30): Promise<any> {
    try {
      ...
      const messageStats = this.getQuery(
        `
          SELECT 
      ...
      const responseStats = this.getQuery(
        `
          SELECT 
      ...
      const categoryStats = this.allQuery(
        `
          SELECT 
            category,
            COUNT(*) as count,

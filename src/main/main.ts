    ...
    ipcMain.handle("ai:generateResponse", async (event, requestData) => {
      try {
        ...
        try {
          ...
          this.logger.debug("Could not fetch documents:", error);
        }
        ...
        try {
          ...
          this.logger.debug("Could not fetch user devices:", error);
        }
    ...
    ipcMain.handle("ai:sendChatMessage", async (event, requestData) => {
      try {
        ...
        const response = await this.aiService.generateResponse(
          ...
          modelId,
          content,
          images // Pass images for vision models
        );
  ...
  private async initialize(): Promise<void> {
    try {
      ...
      app.setAboutPanelOptions({
        applicationName: "AI Helpdesk",
        applicationVersion: "1.0.0",
        copyright: "© 2025 Toby Miller",
        authors: ["Toby Miller"],
        website: "https://github.com/bagelhole/ai-helpdesk",

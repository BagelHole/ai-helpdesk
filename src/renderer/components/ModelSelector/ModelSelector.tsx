...
interface ModelSelectorProps {
  ...
  dropUp?: boolean; // Controls dropdown direction
}
    ...
    <div className={`relative ${className}`}>
      ...
      <div className="relative">
        <button
          ...
          className="relative w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-md shadow-sm pl-2 pr-6 py-1 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 text-xs hover:border-gray-300 dark:hover:border-gray-500 transition-colors"
        ...
        {isOpen && (
          <div
            className={`absolute z-10 w-full max-w-xs bg-white/95 dark:bg-gray-800/95 backdrop-blur-sm shadow-xl max-h-64 rounded-lg py-1 text-xs ring-1 ring-gray-200/50 dark:ring-gray-700/50 overflow-auto focus:outline-none border border-gray-200/50 dark:border-gray-700/50 ${
          ...
          >
            {Object.entries(availableModels).map(([providerId, models]) => {
              ...
              return (
                <div key={providerId}>
                  {/* Provider header */}
                  <div className="px-3 py-1.5 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide bg-gray-50/50 dark:bg-gray-700/50">
                    {provider.name}
                  </div>
                  ...
                  {models.map((model) => (
                    <button
                      ...
                      className={`relative cursor-pointer select-none py-1.5 pl-3 pr-8 w-full text-left hover:bg-blue-50/50 dark:hover:bg-gray-600/50 transition-colors ${
                        selectedProviderId === providerId &&
                        selectedModelId === model.id
                          ? "bg-blue-100/50 dark:bg-blue-900/30 text-blue-900 dark:text-blue-100"
